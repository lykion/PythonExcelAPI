...
def test_case_in_excel(test_case_file):
    ...
    for i in range(1, table.nrows):
        ...
        try:
            ...
            print(e)
            print("第{}个接口请求失败，请检查接口是否异常.".format(api_id))
...
def interface_test(api_id, api_name, api_host, api_url, api_method, api_data_type, api_request_data, api_check_point):
    ...
    if api_method == "GET":
        ...
        if bool(api_request_data) == False:
            ...
            if status_code == 200:
                log.info("第{}条用例-{}-执行成功，状态码为:{}，结果返回值为--{}.".format(api_id, api_name, status_code, res.text))
            else:
                log.error("第{}条用例-{}-执行失败！！！错误返回码:{}".format(api_id, api_name, status_code))
        else:
